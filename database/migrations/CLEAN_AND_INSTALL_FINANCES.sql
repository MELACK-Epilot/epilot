-- ============================================================================
-- NETTOYAGE COMPLET ET INSTALLATION DES FINANCES
-- Script tout-en-un : Supprime l'ancien, installe le nouveau
-- Date : 2025-11-05
-- ============================================================================
...
DO $$ 
BEGIN
  RAISE NOTICE '🧹 Nettoyage des anciennes structures...';
END $$;
...
SELECT cron.unschedule('refresh-financial-views') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'refresh-financial-views');
SELECT cron.unschedule('create-daily-snapshot') WHERE EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'create-daily-snapshot');
...
DO $$ 
BEGIN
  RAISE NOTICE '📊 Création des tables financières...';
END $$;
...
DO $$ 
BEGIN
  RAISE NOTICE '📊 Création des vues matérialisées...';
END $$;
...
-- Index APRÈS création de la vue
...
BEGIN
  ...
  RAISE NOTICE '📊 4 vues matérialisées créées';
