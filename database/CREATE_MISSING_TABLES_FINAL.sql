...
-- 1. Ajouter school_group_id à activity_logs
ALTER TABLE activity_logs 
ADD COLUMN IF NOT EXISTS school_group_id UUID;

CREATE INDEX IF NOT EXISTS idx_activity_logs_school_group_id 
ON activity_logs(school_group_id);

-- 2. Créer system_alerts
...
CREATE INDEX IF NOT EXISTS idx_system_alerts_school_group_id 
ON system_alerts(school_group_id);

CREATE INDEX IF NOT EXISTS idx_system_alerts_is_read 
ON system_alerts(is_read);

CREATE INDEX IF NOT EXISTS idx_system_alerts_severity 
ON system_alerts(severity);

-- 3. Créer fee_payments
...
CREATE INDEX IF NOT EXISTS idx_fee_payments_school_group_id 
ON fee_payments(school_group_id);
...
CREATE INDEX IF NOT EXISTS idx_fee_payments_due_date 
ON fee_payments(due_date);

-- 4. Désactiver RLS (pour développement)
