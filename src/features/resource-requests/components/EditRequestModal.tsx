/**
 * Modal de modification d'une demande de ressources
 * Permet de modifier une demande en attente
 */
...
export const EditRequestModal = ({
  ...
  const handleSubmit = async () => {
    if (!request || !formData.title || !formData.school_id || items.length === 0) {
      return;
    }
    ...
    try {
      await onSubmit(request.id, {
        ...
        items: items.map(({ id, ...item }) => ({
          ...item,
          // Garder l'ID si c'est un item existant, sinon undefined pour créer un nouveau
          ...
          {items.length > 0 && (
            <div className="space-y-4 border-t pt-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-lg">Ressources ({items.length})</h3>
