...
import { 
  ...
  UserCheck,
  Bot,
  // Icônes pour le mapping complet
  CreditCard,
  FolderOpen,
  UserPlus,
  ListChecks,
...
export const MyModulesProviseurModern = () => {
  ...
  const modulesEnrichis = useMemo((): ModuleEnrichi[] => {
    return modules.map(module => {
      ...
      // 1. Essayer d'utiliser le nom d'icône venant de la base (CheckCircle, CreditCard, FileText...)
...
// Mapping COMPLET des noms d'icônes venant de la base de données
// Basé sur la table modules.icon de Supabase
function mapIconNameToComponent(iconName: string | null): React.ReactNode | null {
  if (!iconName) return null;

  // Normaliser le nom (enlever espaces, tirets, mettre en minuscules)
  const normalized = iconName.replace(/[^a-zA-Z]/g, '').toLowerCase();

  // Mapping exact depuis la base de données
